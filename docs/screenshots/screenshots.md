# Monkey Playground - Screenshots

Here are some visual impressions of the current monkey playground app and its features.

## Snippet Editor (dark-theme)

![Snippet Editor dark-theme screenshot](./snippet-page-dark.png)

## Snippet Editor (light-theme)

![Snippet Editor light-theme screenshot](./snippet-page-light.png)

## Saved Snippets Sidebar

![Saved Snippets Sidebar screenshot](./saved-snippets-sidebar.png)

## Delete Snippet Confirmation Dialog

![Delete Snippet Confirmation Dialog screenshot](./delete-snippet-confirmation-dialog.png)

## Share Snippet Dialog

![Share Snippet Dialog screenshot](./share-snippet-dialog.png)

## Shared Snippet View for anonymous user

![Shared Snippet View for anonymous user screenshot](./shared-snippet-anon-user.png)
